...
async def get_live_positions(request: Request):
    """Get live open positions with TP/SL levels from Kraken"""
    api_key = request.headers.get("X-API-Key") or request.query_params.get("key")
    ...
    try:
        ...
        # Get user with exchange credentials
        user = await conn.fetchrow("""
            SELECT id, email, exchange, exchange_api_key, exchange_api_secret 
            FROM follower_users 
            WHERE api_key = $1
        """, api_key)
        ...
        if not user['exchange_api_key'] or not user['exchange_api_secret']:
            return {
                "status": "not_configured",
                "message": "Exchange API not configured",
                "positions": []
            }
        
        # Initialize Kraken Futures exchange
        exchange = ccxt.krakenfutures({
            'apiKey': user['exchange_api_key'],
            'secret': user['exchange_api_secret'],
            'enableRateLimit': True
        })
        ...
        # Filter for actual open positions (non-zero)
        open_positions = []
        for pos in positions:
            ...
            if contracts != 0:
                ...
                # Fetch open orders for this position to find TP/SL
                tp_order = None
                sl_order = None
                
                try:
                    # Map symbol for Kraken (ADA/USD:USD format)
                    ...
                    for order in open_orders:
                        ...
                        # Determine if this is TP or SL based on order type and prices
                        if side == 'long':
                            # For long: TP is sell above entry, SL is sell below entry
                            if order_side == 'sell':
                                if trigger_price and float(trigger_price) < entry_price:
                                    sl_order = {
                                        'id': order.get('id'),
                                        'price': float(trigger_price),
                                        'type': order_type
                                    }
                                elif limit_price and float(limit_price) > entry_price:
                                    tp_order = {
                                        'id': order.get('id'),
                            ...
                            # For short: TP is buy below entry, SL is buy above entry
                            if order_side == 'buy':
                                if trigger_price and float(trigger_price) > entry_price:
                                    sl_order = {
                                        'id': order.get('id'),
                    ...
                    print(f"Error fetching orders for {symbol}: {order_err}")
